...
#| context: setup
# ---- Load data ----
YELL_Fire <- read_csv(path(wDir, "data/YELL_Fires_2025.csv")) # 2024
INFORM_Fire <- read_csv(path(wDir, "data/Yellowstone_INFORM_Incidents_2025.csv")) # 2024

# For code diagnostics (will not resolve in Shiny runtime)
#YELL_Fire <- read_csv(file = file.path(wDir, "data", "YELL_Fires_2025.csv"))
#INFORM_Fire <- read_csv(file = file.path(wDir, "data", "Yellowstone_INFORM_Incidents_2025.csv"))

# ---- Collect file date into new objects ----
file_info <- file.info(path(wDir,"data/YELL_Fires_2025.csv"))
...
INFORM_Fire <- INFORM_Fire %>%
  filter(!tolower(POOProtectingAgency) %in% "usfs") %>%
  mutate(IncidentName = clean_name(IncidentName))
...
#write.csv(YELL_Fire_Final, "YELL_Fire_Final_Merged_2025_Updated.csv")
```


```{r batch choices setup}
#| context: setup
#| include: false

# Precompute batch choices/labels once so they're available to both UI and server
batch_lengths <- c(5, 10, 15, 20, 25, 30, 40, 50)
...
batch_choices[["1989-Current"]] <- sort(
  unique(YELL_Fire_Final$Year[YELL_Fire_Final$Year >= 1989]),
  decreasing = TRUE)

batch_choices[["Full Record (1972-Current)"]] <- sort(
  unique(YELL_Fire_Final$Year[YELL_Fire_Final$Year >= 1972]), 
  decreasing = TRUE)
...
```{r summary header server, include=FALSE, echo=FALSE}
#| context: server
output$summary_header <- renderUI({
  ...
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
  }
...
```{r summary header ui, echo=FALSE}
...
output$hist_acres <- renderPlotly({
  req(input$selected_batch == "Full Record (1972-Current)")

  df <- YELL_Fire_Final %>%
    filter(Year %in% batch_choices[["Full Record (1972-Current)"]]) %>%
...
```{r summary header for summary table server, include=FALSE, echo=FALSE}
#| context: server
...
```{r summary header for summary table ui, echo=FALSE}
uiOutput("summary_table_header")
```


```{r total acres server, include=FALSE, echo=FALSE}
#| context: server
# --- Helper function: robust numeric mode (optionally bin/round) ---
get_mode_num <- function(x, digits = 2) {
  x <- suppressWarnings(as.numeric(x))
  x <- x[!is.na(x)]
  if (length(x) == 0) return(NA_real_)

  # Round to stabilize float noise and align with how acres are recorded (e.g., 0.01, 0.1, etc.)
...
summary_df <- reactive({
  ...
  df <- YELL_Fire_Final %>%
    ...
    dplyr::mutate(Acres = suppressWarnings(as.numeric(Acres))) %>%
    dplyr::filter(!is.na(Acres))

  total_number <- nrow(df)
  total_acres  <- sum(df$Acres, na.rm = TRUE)

  yearly_summary <- df %>%
    dplyr::group_by(Year) %>%
    dplyr::summarise(
      Total_Acres_Yearly = sum(Acres, na.rm = TRUE),
      Fire_Count_Yearly  = dplyr::n(),
      .groups = "drop"
    )

  # ----- ACRES -----
  if (length(yrs) == 1) {
    # Single-year: per-fire stats
    acres_avg_val    <- mean(df$Acres, na.rm = TRUE)
    acres_median_val <- median(df$Acres, na.rm = TRUE)
  } else {
    # Multi-year: stats across yearly totals (your desired behavior)
    acres_avg_val    <- mean(yearly_summary$Total_Acres_Yearly, na.rm = TRUE)
    acres_median_val <- median(yearly_summary$Total_Acres_Yearly, na.rm = TRUE)
  }

  # Mode: ALWAYS incident-level acres (matches your old “0.1” behavior)
  acres_mode_val <- get_mode_num(df$Acres, digits = 2)

  # ----- FIRES -----
  if (length(yrs) == 1) {
    fires_avg_val    <- total_number
    fires_median_val <- total_number
    fires_mode_val   <- total_number
  } else {
    fires_avg_val    <- mean(yearly_summary$Fire_Count_Yearly, na.rm = TRUE)
    fires_median_val <- median(yearly_summary$Fire_Count_Yearly, na.rm = TRUE)

    # fire count mode across yearly fire counts
    tab_f <- table(yearly_summary$Fire_Count_Yearly)
    fires_mode_val <- as.numeric(names(tab_f)[which.max(tab_f)])
  }

  # ---- Build final table with rounding rules ----
  tibble::tibble(
    `Number of Fires (Total)`   = as.integer(round(total_number, 0)),
    `Acres Burned (Total)`      = round(total_acres, 0),

    `Number of Fires (Average)` = as.integer(round(fires_avg_val, 0)),
    `Acres Burned (Average)`    = round(acres_avg_val, 0),

    `Number of Fires (Median)`  = as.integer(round(fires_median_val, 0)),
    `Acres Burned (Median)`     = round(acres_median_val, 1),

    `Number of Fires (Mode)`    = as.integer(round(fires_mode_val, 0)),
    `Acres Burned (Mode)`       = round(acres_mode_val, 2)  # <-- keep 2 decimals ONLY here
  ) %>%
...
output$plot_acres <- renderPlotly({
  ...
  p <- ggplot(df, aes(x = factor(Label, levels = Label), y = Total_Acres,
        ...
        scale_y_continuous(labels = scales::comma) +  # <- adds 1,000 separators
        theme_minimal(base_size = 16) +
        theme(axis.text.x = element_text(angle = 90, hjust = 1),
              axis.title.x = element_text(face = "bold", size = 14),
              axis.title.y = element_text(face = "bold", size = 14)) +
...
output$total_acres_ui <- renderUI({
  yrs <- selected_years()
  ...
  tagList(
    reactable::reactable(
      ...
      columns = list(
        ...
        Value = colDef(
          name = "",
          format = colFormat(separators = TRUE), # <- adds 1,000 separators
...
```{r total acres ui, echo=FALSE}
uiOutput("total_acres_ui")
```
...
decadal_plot_df <- reactive({
  ...
  letters_df <- if (length(unique(df$Decade)) > 1) {
    dunn_result <- dunnTest(Acres ~ Decade, data = df, method = "holm")
    ...
    multcompView::multcompLetters(pval_matrix)$Letters %>%
      ...
      left_join(
        df %>% group_by(Decade) %>% summarise(max_y = max(Acres), .groups = "drop"),
        by = "Decade"
      ) %>%
...
# Helper: build ggplot for PLOTLY (includes tooltip text)
make_decadal_boxplot_gg_plotly <- function(df, letters_df) {
  ggplot(
    df,
    aes(
      x = Decade,
      y = log10(Acres),
      text = paste0(
        "Decade: ", gsub("_", "–", as.character(Decade)),
        "<br>Acres burned: ", scales::comma(round(Acres, 0)),
        "<br>Log10(acres): ", round(log10(Acres), 3)
      )
    )
  ) +
    ...
    geom_text(
      data = letters_df,
      aes(x = Decade, y = y_pos, label = Letters),
      inherit.aes = FALSE,
      size = 6, vjust = 0, color = "red3"
    ) +
    scale_x_discrete(labels = function(x) gsub("_", "–", x)) +
    scale_y_continuous(name = "Log(Acres Burned)") +
    theme_minimal(base_size = 16) +
    theme(
      axis.title.x = element_text(face = "bold", size = 14),
      axis.title.y = element_text(face = "bold", size = 14)
      # NOTE: caption handled in UI below plot, so no caption theme needed here
    ) +
    labs(
      title = NULL,
      x = "Decade",
...
# Helper: build ggplot for PNG (NO tooltip text aesthetic)
make_decadal_boxplot_gg_png <- function(df, letters_df, title = "Acres Burned by Decade") {
  caption_txt <- "Letters indicate statistically distinct groups (Dunn’s test, Holm correction)"

  ggplot(
    df,
    aes(x = Decade, y = log10(Acres))
  ) +
    geom_boxplot(fill = "#2a9d8f", alpha = 0.7) +
    geom_jitter(width = 0.1, alpha = 0.3, color = "gray40", size = 4) +
    geom_text(
    ...
    theme(
      ...
      plot.caption = element_text(hjust = 0, face = "italic", color = "red", size = 10)
    ) +
    labs(
      title = title,
      x = "Decade",
...
# Render the decadal boxplot as Plotly (no title inside plot)
output$decadal_boxplot <- renderPlotly({
  ...
  g <- make_decadal_boxplot_gg_plotly(df, letters_df)
...
# Conditional UI: header + plotly output + caption (HTML) (only when full record selected)
output$decadal_boxplot_ui <- renderUI({
  req(input$selected_batch == "Full Record (1972-Current)")

  tagList(
    h4(tags$span(
      style = "font-weight: bold; color: purple; font-size: 24px;",
      "Acres Burned by Decade"
    )),
    plotlyOutput("decadal_boxplot", height = "500px"),
    tags$p(
      style = "color: red; font-style: italic; text-align: left; margin-top: 6px;",
      "Letters indicate statistically distinct groups (Dunn’s test, Holm correction)"
    )
  )
})

# Download button (PNG keeps the title inside the plot)
output$download_decadal_boxplot <- downloadHandler(
  ...
  content = function(file) {
    ...
    g <- make_decadal_boxplot_gg_png(df, letters_df, title = "Acres Burned by Decade")
...
conditionalPanel(
  condition="input.selected_batch=='Full Record (1972-Current)'",
  downloadButton("download_decadal_boxplot", "Download Plot")
)
```


```{r discovery dates, include=FALSE}
#| context: server
# ---- Shared data prep (returns filtered_df + letters_df) ----
discovery_doy_data <- function(include_1988 = FALSE) {
  ...
  filtered_df <- YELL_Fire_Final %>%
    ...
    filter(Year >= 1972) %>%
    { if (!include_1988) filter(., Year != 1988) else . }
  ...
  dunn_result <- dunnTest(DOY ~ Decade, data = filtered_df, method = "holm")
  ...
  letters_df <- multcompLetters(pval_matrix)$Letters |>
    enframe(name = "Decade", value = "Letters")
  ...
  letters_df <- left_join(letters_df, position_df, by = "Decade") %>%
    mutate(label_y = max_y + 5)

  list(filtered_df = filtered_df, letters_df = letters_df)
}

# ---- Plot builder for PLOTLY (includes tooltip text aesthetic) ----
make_discovery_doy_gg_plotly <- function(include_1988 = FALSE) {
  dat <- discovery_doy_data(include_1988 = include_1988)
  filtered_df <- dat$filtered_df
  letters_df  <- dat$letters_df
  ...
  ggplot(
    ...
    labs(
      title = NULL,         # no title inside the displayed plot
      x = "Decade",
      y = "Day of Year"
    )
}

# ---- Plot builder for PNG (NO tooltip aesthetic; keeps title + caption) ----
make_discovery_doy_gg_png <- function(include_1988 = FALSE,
...
# ---- Render displayed plot as Plotly (no title inside plot) ----
output$discovery_doy_plot <- renderPlotly({
  req(input$selected_batch == "Full Record (1972-Current)")

  g <- make_discovery_doy_gg_plotly(include_1988 = isTRUE(input$include_1988))
...
# ---- Conditional UI: header + plotly output + caption (HTML) ----
output$discovery_doy_ui <- shiny::renderUI({
  req(input$selected_batch == "Full Record (1972-Current)")

  tagList(
    h4(shiny::tags$span(
      style = "font-weight: bold; color: purple; font-size: 24px;",
      "Fire Discovery Dates by Decade"
    )),
    plotlyOutput("discovery_doy_plot", height = "600px"),
    tags$p(
      style = "color: red; font-style: italic; text-align: left; margin-top: 6px",
      "Letters indicate statistically distinct groups (Dunn’s test, Holm correction)"
    )
...
# ---- Download handler: PNG uses the clean PNG builder ----
output$download_discovery_doy_plot <- shiny::downloadHandler(
  filename = function() {
    paste0("Fire_Discovery_Dates_by_Decade_", Sys.Date(), ".png")
  },
  content = function(file) {
    req(input$selected_batch == "Full Record (1972-Current)")

    g <- make_discovery_doy_gg_png(
      include_1988 = isTRUE(input$include_1988),
      title = "Fire Discovery Dates by Decade"
    )
...
uiOutput("discovery_doy_ui")
conditionalPanel(
  condition = "input.selected_batch == 'Full Record (1972-Current)'",
  downloadButton("download_discovery_doy_plot", "Download Plot")
)
```
...
```{r summary header for size class server, include=FALSE, echo=FALSE}
#| context: server
output$size_class_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for size class ui, echo=FALSE}
uiOutput("size_class_header")
```

```{r summary size class server, include=FALSE, echo=FALSE}
#| context: server
output$size_class_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
```{r summary size class ui, echo=FALSE}
uiOutput("size_class_ui")
```
...
```{r summary header for management action server, include=FALSE, echo=FALSE}
#| context: server
output$management_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for management action ui, echo=FALSE}
uiOutput("management_header")
```

```{r summary type server, include=FALSE, echo=FALSE}
#| context: server
output$management_ui <- renderUI({
  ...
  summary_type <- YELL_Fire_Final %>%
    ...
    mutate(Type_Description = case_when(
      Type == 11 ~ "Direct Extinguishment",
      Type %in% c(14, 49) ~ "Indirect Confinement",
      Type == 21 ~ "Natural Out",
      Type == 48 ~ "Rx Burn",
...
```{r summary type ui, echo=FALSE}
uiOutput("management_ui")
```
...
```{r summary header for fire cause server, include=FALSE, echo=FALSE}
#| context: server
output$cause_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for fire cause ui, echo=FALSE}
uiOutput("cause_header")
```

```{r summary cause server, include=FALSE, echo=FALSE}
#| context: server
output$cause_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
```{r summary header for specific cause server, include=FALSE, echo=FALSE}
#| context: server
output$specific_cause_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for specific cause ui, echo=FALSE}
uiOutput("specific_cause_header")
```

```{r summary cause specific server, include=FALSE, echo=FALSE}
#| context: server
output$specific_cause_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
```{r summary cause specific ui, echo=FALSE}
uiOutput("specific_cause_ui")
```
...
```{r summary header for fires by month server, include=FALSE, echo=FALSE}
#| context: server
output$month_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for fires by month ui, echo=FALSE}
uiOutput("month_header")
```
...
output$monthly_fire_histograms_avg <- renderPlotly({
  ...
  g1 <- make_bar(fire_counts_avg, Month, Avg_Fires, "Avg. Fires", "#264653")
  g2 <- make_bar(acre_totals_avg, Month, Avg_Acres, "Avg. Acres Burned", "#e76f51")
...
output$monthly_fire_histograms_total <- renderPlotly({
  ...
  g3 <- make_bar(fire_counts_total, Month, Total_Fires, "Total Fires", "#2a9d8f")
  g4 <- make_bar(acre_totals_total, Month, Total_Acres, "Total Acres Burned", "#f4a261")
...
output$download_monthly_plots <- downloadHandler(
  filename = function() "YELL_Monthly_Fires.png",
  content = function(file) {
    req(selected_years())
    ...
    ggplot2::ggsave(
      file,
      plot = patchwork::wrap_plots(g_png_1, g_png_2, g_png_3, g_png_4, ncol = 2),
      width = 12, height = 8, dpi = 300
    )
  }
)

# ---- Render UI with section headers (HIDDEN until years selected) ----
output$monthly_fire_histograms_ui <- renderUI({
  req(selected_years())  # <-- THIS is the key fix

  tagList(
    tags$h4("Average Number of Fires and Acres Burned by Month",
            style = "color: purple; font-weight: bold; margin-top: 20px;"),
    plotlyOutput("monthly_fire_histograms_avg", height = "400px"),

    tags$h4("Total Number of Fires and Acres Burned by Month",
            style = "color: purple; font-weight: bold; margin-top: 20px;"),
...
```{r summary header for declared out server, include=FALSE, echo=FALSE}
#| context: server
output$declared_out_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for declared out ui, echo=FALSE}
uiOutput("declared_out_header")
```

```{r time to declared out server, include=FALSE, echo=FALSE}
#| context: server
output$declared_out_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
```{r time to declared out ui, echo=FALSE}
uiOutput("declared_out_ui")
```
...
```{r summary header for declared contained server, include=FALSE, echo=FALSE}
#| context: server
output$declared_contained_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for declared contained ui, echo=FALSE}
uiOutput("declared_contained_header")
```

```{r time to declared contained server, include=FALSE, echo=FALSE}
#| context: server
output$declared_contained_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
  ...
  tagList(
    ...
    tags$p(
      style = "color: red; font-style: italic; margin-top: 10px;",
      "Declared contained dates are not available prior to 1992."
    ),
...
```{r time to declared contained ui, echo=FALSE}
uiOutput("declared_contained_ui")
```
...
```{r summary header for declared controlled server, include=FALSE, echo=FALSE}
#| context: server
output$declared_controlled_header <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or multi-year group to view results."))
...
```{r summary header for declared controlled ui, echo=FALSE}
uiOutput("declared_controlled_header")
```

```{r time to declared controlled server, include=FALSE, echo=FALSE}
#| context: server
output$declared_controlled_ui <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
```{r time to declared controlled ui, echo=FALSE}
uiOutput("declared_controlled_ui")
```
