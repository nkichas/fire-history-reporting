...
output$decadal_boxplot <- renderPlot({
  ...
  ggplot(df, aes(x=Decade, y=log10(Acres))) +
    ...
    theme_minimal(base_size=16) +
    labs(title="Acres Burned by Decade",
...
#| context: server
# Plot factory (reused by renderPlot + download)
make_discovery_doy_ggplot <- function(include_1988 = FALSE) {
  ...
  filtered_df <- YELL_Fire_Final %>%
    ...
    { if (!include_1988) dplyr::filter(., Year != 1988) else . }

  breaks <- seq(1972, max(filtered_df$Year, na.rm = TRUE) + 11, by = 11)
  labels <- paste0(breaks[-length(breaks)], "_", breaks[-1] - 1)

  filtered_df <- filtered_df %>%
    dplyr::mutate(Decade = cut(Year, breaks = breaks, labels = labels, include.lowest = TRUE))
  ...
  position_df <- filtered_df %>%
    ...
    dplyr::summarize(max_y = max(DOY, na.rm = TRUE), .groups = "drop")

  letters_df <- dplyr::left_join(letters_df, position_df, by = "Decade") %>%
    dplyr::mutate(label_y = max_y + 5)

  ggplot2::ggplot(filtered_df, ggplot2::aes(x = Decade, y = DOY)) +
    ggplot2::geom_boxplot(fill = "#264653", alpha = 0.7) +
    ggplot2::geom_jitter(width = 0.1, alpha = 0.3, color = "gray40", size = 3) +
    ggplot2::geom_text(
      data = letters_df,
      ggplot2::aes(x = Decade, y = label_y, label = Letters),
      size = 6, vjust = 0, color = "red3"
    ) +
    ggplot2::scale_x_discrete(labels = function(x) gsub("_", "–", x)) +
    ggplot2::scale_y_continuous(name = "Day of Year", breaks = seq(100, 300, 20)) +
...
# Render plot (only when full record selected)
output$discovery_doy_plot <- renderPlot({
  req(input$selected_batch == "Full Record (Post-1972)")
  make_discovery_doy_ggplot(include_1988 = isTRUE(input$include_1988))
})

# Conditional UI: only show the plot (no button here)
output$discovery_doy_ui <- renderUI({
  req(input$selected_batch == "Full Record (Post-1972)")
  plotOutput("discovery_doy_plot", height = "600px")
})

# Download handler (button is placed in the document body via conditionalPanel)
output$download_discovery_doy_plot <- downloadHandler(
  ...
  content = function(file) {
    req(input$selected_batch == "Full Record (Post-1972)")
    p <- make_discovery_doy_ggplot(include_1988 = isTRUE(input$include_1988))
    ggplot2::ggsave(
      filename = file,
      plot = p,
      device = "png",
