...
# Helper: build the ggplot object (optionally with a title for the PNG)
make_decadal_boxplot_gg <- function(df, letters_df, title = NULL) {
  ...
  ggplot(
    ...
    labs(
      title = title,          # NULL for displayed; set for PNG
      x = "Decade",
      y = "Log(Acres Burned)",
      caption = caption_txt
    )
...
output$decadal_boxplot <- renderPlotly({
  ...
  g <- make_decadal_boxplot_gg(df, letters_df, title = NULL)

  ggplotly(g, tooltip = "text") %>%
    layout(
      hoverlabel = list(bgcolor = "white"),
      hovermode = "closest",
      # We render caption as HTML below, so remove plotly's caption area spacing
      margin = list(b = 60)
    )
})
...
output$decadal_boxplot_ui <- renderUI({
  ...
  tagList(
    ...
    tags$p(
      style = "color: red; font-style: italic; font-size: 10px; text-align: left; margin-top: 6px;",
...
output$download_decadal_boxplot <- downloadHandler(
  ...
  content = function(file) {
    ...
    g <- make_decadal_boxplot_gg(df, letters_df, title = "Acres Burned by Decade")
...
#| context: server
# Helper: build ggplot (optionally include a title for PNG)
make_discovery_doy_ggplot <- function(include_1988 = FALSE, title = NULL) {
  ...
  letters_df <- dplyr::left_join(letters_df, position_df, by = "Decade") %>%
...
# Conditional UI: header + plotly output + caption (HTML) (only when full record selected)
output$discovery_doy_ui <- renderUI({
  req(input$selected_batch == "Full Record (1972-Current)")
...
# Download handler (PNG keeps the title inside the plot)
output$download_discovery_doy_plot <- downloadHandler(
  filename = function() {
