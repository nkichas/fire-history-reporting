...
# ---- Load data ----
YELL_Fire <- read_csv(path(wDir, "data/YELL_Fires_2025.csv")) # 2025
INFORM_Fire <- read_csv(path(wDir, "data/Yellowstone_INFORM_Incidents_2025.csv")) # 2025
...
```{r summary header server, include=FALSE}
...
```{r full record plots ui, include=TRUE, echo=FALSE}
shiny::conditionalPanel(
  condition = "input.selected_batch == 'Full Record (Post-1972)'",
  plotlyOutput("hist_acres", height = "450px"),
  plotlyOutput("hist_fires", height = "450px")
)
```
...
uiOutput("summary_header_for_summary_table")
```

```{r summary header for summary table server, include=FALSE}
output$summary_header_for_summary_table <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("summary_header_for_size_class")
```

```{r summary header for size class server, include=FALSE}
output$summary_header_for_size_class <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("summary_size_class")
```

```{r summary size class server, include=FALSE}
output$summary_size_class <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
uiOutput("summary_header_for_management_action")
```

```{r summary header for management action server, include=FALSE}
output$summary_header_for_management_action <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
```{r summary type server, include=FALSE}
...
uiOutput("summary_header_for_fire_cause")
```

```{r summary header for fire cause server, include=FALSE}
output$summary_header_for_fire_cause <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("summary_cause")
```

```{r summary cause server, include=FALSE}
output$summary_cause <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
uiOutput("summary_header_for_specific_cause")
```

```{r summary header for specific cause server, include=FALSE}
output$summary_header_for_specific_cause <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("summary_cause_specific")
```

```{r summary cause specific server, include=FALSE}
output$summary_cause_specific <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
uiOutput("summary_header_for_fires_by_month")
```

```{r summary header for fires by month server, include=FALSE}
output$summary_header_for_fires_by_month <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("summary_header_for_declared_out")
```

```{r summary header for declared out server, include=FALSE}
output$summary_header_for_declared_out <- renderUI({
  yrs <- sort(as.numeric(selected_years()))
  if (is.null(yrs) || length(yrs) == 0) {
    return(tags$div(style = "color: red; font-style: italic;",
                    "Please select either individual year(s) or a year group to view results."))
...
uiOutput("time_to_declared_out")
```

```{r time to declared out server, include=FALSE}
output$time_to_declared_out <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
uiOutput("time_to_declared_contained")
```

```{r time to declared contained server, include=FALSE}
output$time_to_declared_contained <- renderUI({
  yrs <- selected_years()
  req(yrs)
...
uiOutput("time_to_declared_controlled")
```

```{r time to declared controlled server, include=FALSE}
output$time_to_declared_controlled <- renderUI({
  yrs <- selected_years()
  req(yrs)
